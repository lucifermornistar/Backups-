	...
	try {
		...
		for (let nomea of nomes_arquivos) {
			if (!nomea.endsWith('.js')) {
				//qdo for pasta (nunca colocar txt nas pastas de sistemas e comandos)
			...
			try {
				...
				if (bot[nomea]) console.log('Tem 2 .js de mesmo nome: ' + nomea);
				...
				if (pasta.includes('comandos/')) {
					...
					if (!bot.listas_comandos[subpasta])
						//se não tem a lista pra essa subpasta
...
try {
	//faz a leitura dos eventos relativos a API discord.js
